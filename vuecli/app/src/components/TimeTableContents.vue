...
export default {
  ...
  methods: {
    ...
    async fetchItems() {
      try {
        const itemsSnapshot = await getDocs(query(collection(db,'users', this.uid ,'scheduledItems'), where('role', '==', '時間割')));
        const batch = itemsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
